...
void topicCorpus::saveTopicModel()
{
    int topicIndex;
    int wordIndex;
    printf("Normalizing word probabilities in final topic model");

    for( topicIndex = 0; topicIndex < K; topicIndex++) {
        float sum = 0;
        printf("TOPIC %d\n", topicIndex);
        printf("STEP 1 OUT 2: Exponentiating word probabilities for all models\n");
        for( wordIndex = 0; wordIndex < nWords; wordIndex++) {
            double expVal = exp( topicWords[wordIndex][topicIndex] );
            topicWords[wordIndex][topicIndex] = expVal;
            sum += expVal;
	}
	printf("STEP 2 OUT OF 2: Normalizing value through division by sum\n");
        for(  wordIndex = 0; wordIndex < nWords; wordIndex++) {
            topicWords[wordIndex][topicIndex] = topicWords[wordIndex][topicIndex]/sum; 
	}
    ...
    printf("SERIALIZING MODEL\n");

 
     
    FILE* f = fopen_("./savedModel.txt", "w");
    for (int topicIndex = 0; topicIndex < K; topicIndex++)
    {
        double sum = 0;
        vector < pair<double, int> > bestWords;
        for (int wordIndex = 0; wordIndex < nWords; wordIndex++)
          bestWords.push_back(pair<double, int> (-topicWords[wordIndex][topicIndex], wordIndex));
        sort(bestWords.begin(), bestWords.end());

        for (int wordIndex = 0; wordIndex < nWords; wordIndex++){
          fprintf(f, "%s %f ", corp->idWord[bestWords[wordIndex].second].c_str(), -bestWords[wordIndex].first);
          sum += bestWords[wordIndex].first;
        }
	fprintf(f,"\n");
        printf("SUM IS %f\n", sum);
     }
